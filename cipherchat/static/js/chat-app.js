(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Targeted presence updates for contacts/DM partners/group-mates (not a
    // platform-wide broadcast — see server-side chat/events.py).
    socket.on('presence_update', function (data) {
      if (!data || !data.email) return;
      document.querySelectorAll('[data-email="' + data.email + '"] .contact-avatar, [data-email="' + data.email + '"] .convo-avatar')
        .forEach(function (el) { el.classList.toggle('is-online', !!data.online); });
      if (openProfileEmail === data.email) applyPresenceToHeader(data);
    });

    socket.on('user_presence', function (data) {
      if (!data || !data.email) return;
      if (currentRoomName && currentRoomName === activePrivateTargetRoom(data.email)) {
        applyPresenceToHeader(data);
      }
      document.querySelectorAll('[data-email="' + data.email + '"] .contact-avatar, [data-email="' + data.email + '"] .convo-avatar')
        .forEach(function (el) { el.classList.toggle('is-online', !!data.online); });
    });

    let openProfileEmail = null;
    let currentPrivateTarget = null;

    function activePrivateTargetRoom(email) {
      // Best-effort: only used to decide whether a presence event matches
      // the conversation currently open in the header.
      return currentPrivateTarget === email ? currentRoomName : null;
    }

    function applyPresenceToHeader(data) {
      if (!currentChatStatus) return;
      if (data.online) {
        currentChatStatus.textContent = 'Online';
      } else if (data.last_seen) {
        currentChatStatus.textContent = 'Last seen ' + formatTime(data.last_seen);
      } else {
    ...
    function getDisplayNameForEmail(email) {
      if (!email) return 'Private chat';
      const match = activeUsers.find(function (u) { return u.email === email; });
    ...
    socket.on('private_chat_ready', function (data) {
      ...
      const otherName = typeof other === 'string' ? getDisplayNameForEmail(other) : 'Private chat';
      let convo = conversations.find(function (c) { return c.id === data.room; });
      if (!convo) {
        convo = { id: data.room, name: otherName, email: typeof other === 'string' ? other : undefined, type: 'private', unread: currentRoomName === data.room ? 0 : 1, lastMessage: 'Private chat ready', lastTime: '' };
      ...
      switchConversation(data.room, otherName, 'private');
    });
    ...
    function loadConversations() {
      fetch('/get-contacts', { credentials: 'same-origin' })
        ...
        .then(function (payload) {
          var list = (payload && payload.conversations) || [];
          if (!Array.isArray(list)) list = [];
          conversations = list.map(function (c) {
            return {
              id: c.id,
              type: c.type || 'private',
              name: c.name || (c.email ? (c.email.split('@')[0] || c.email) : c.id),
              email: c.email,
              unread: c.unread || 0,
              lastMessage: c.lastMessage || (c.type === 'group' ? 'Welcome to CipherChat' : 'Private chat ready'),
              lastTime: c.lastTime || '',
            };
          });
          if (!conversations.some(function (c) { return c.id === 'general'; })) {
            conversations.unshift({ id: 'general', name: 'General Chat', type: 'group', unread: 0, lastMessage: 'Welcome to CipherChat', lastTime: '' });
          }
          renderConversations();
        })
        .catch(function () {
    ...
    function openPrivateChat(email, name) {
      currentPrivateTarget = email || null;
      if (email) socket.emit('get_user_presence', { email: email });
    ...
    function closeActiveChat() {
      ...
      socket.emit('leave_room', { room: currentRoomName });
      currentRoomName = null;
      currentPrivateTarget = null;
      clearMessages();
      showEmptyState();
      if (currentChatName) currentChatName.textContent = 'CipherChat';
      if (currentChatStatus) currentChatStatus.textContent = socket.connected ? 'online' : 'Reconnecting…';
      if (currentChatAvatar) currentChatAvatar.innerHTML = '<i class="bi bi-shield-lock-fill"></i>';
      if (messageInput) messageInput.disabled = true;
      if (sendBtn) sendBtn.disabled = true;
      if (chatSettingsBtn) chatSettingsBtn.style.display = 'none';
      if (chatBody) chatBody.classList.remove('details-open');
      document.querySelectorAll('.conversation-item').forEach(function (item) {
        item.classList.remove('active');
      });
      // On mobile the sidebar drawer is hidden while a chat is open — closing
      // the chat should bring the conversation list back into view.
      if (window.matchMedia('(max-width: 800px)').matches && sidebar) {
    ...
    function closeTopmostOverlay() {
      // Returns true if something was closed, so ESC doesn't cascade further.
      var openSheet = document.querySelector('.cc-sheet.open');
      if (openSheet) {
        openSheet.classList.remove('open');
        openSheet.setAttribute('aria-hidden', 'true');
        return true;
      }
      var newChatMenu = document.getElementById('newChatMenu');
      if (newChatMenu && newChatMenu.classList.contains('open')) {
        newChatMenu.classList.remove('open');
        newChatMenu.setAttribute('aria-hidden', 'true');
        return true;
      }
      if (chatBody && chatBody.classList.contains('details-open')) {
        chatBody.classList.remove('details-open');
        return true;
      }
      if (window.matchMedia('(max-width: 800px)').matches && sidebar && sidebar.classList.contains('open')) {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      // A modal/menu/sheet takes priority — ESC closes it instead of the chat.
      if (closeTopmostOverlay()) {
        e.preventDefault();
        return;
      }
      // No modal open: close the active conversation, if any. Does nothing
      // if no chat is currently open (never logs out, never touches the
      // socket connection, never removes anything from the chat list).
      if (currentRoomName) {
        closeActiveChat();
        e.preventDefault();
      }
    ...
    function setupEventListeners() {
          ...
          if (tab === 'contacts') {
            ...
            if (contactsList) {
              ...
              fetch('/get-contacts', { credentials: 'same-origin' })
                .then(function (r) { return r.json(); })
                .then(function (payload) {
                  const list = (payload && payload.contacts) || [];
                  contactsList.innerHTML = (list || []).map(function (u) {
                    var contactName = u.display_name || u.username || u.email;
                    return '<div class="contact-item" data-email="' + escapeHtml(u.email) + '" data-display-name="' + escapeHtml(contactName) + '" data-username="' + escapeHtml(u.username || u.email) + '">' +
                      '<div class="contact-avatar">' + escapeHtml((contactName || '?').charAt(0).toUpperCase()) + '</div>' +
                      '<div class="convo-meta"><div class="convo-name">' + escapeHtml(contactName) + '</div>' +
                  ...
                  contactsList.querySelectorAll('.contact-item').forEach(function (item) {
                    const displayName = item.dataset.displayName || item.dataset.username;
                    item.addEventListener('click', function () { openPrivateChat(item.dataset.email, displayName); });
                  });
                });
            }
          } else {
      ...
      function renderUserSearchResults(listOrPayload) {
        const list = Array.isArray(listOrPayload) ? listOrPayload : (listOrPayload && listOrPayload.results) || [];
        if (!contactsList) return;
        if (conversationsList) conversationsList.style.display = 'none';
        contactsList.style.display = 'block';
        if (!list.length) {
          contactsList.innerHTML = '<div class="current-chat-status p-3">No users found.</div>';
          return;
        }
        contactsList.innerHTML = list.map(function (u) {
          const displayName = u.display_name || u.username || u.email;
          const statusLabel = u.online ? 'Online' : (u.last_seen ? 'Last seen ' + formatTime(u.last_seen) : 'Offline');
          return '<div class="contact-item" data-email="' + escapeHtml(u.email) + '" data-display-name="' + escapeHtml(displayName) + '" data-username="' + escapeHtml(u.username || u.email) + '">' +
            '<div class="contact-avatar' + (u.online ? ' is-online' : '') + '">' + escapeHtml((displayName || '?').charAt(0).toUpperCase()) + '</div>' +
            '<div class="convo-meta"><div class="convo-name">' + escapeHtml(displayName) + '</div>' +
            '<div class="convo-last">' + escapeHtml(statusLabel) + '</div></div></div>';
        }).join('');
        contactsList.querySelectorAll('.contact-item').forEach(function (item) {
          const displayName = item.dataset.displayName || item.dataset.username;
          item.addEventListener('click', function () { openPrivateChat(item.dataset.email, displayName); });
      ...
      if (globalSearchInput) {
        globalSearchInput.addEventListener('input', function () {
          ...
          if (!q) {
            // Fall back to the normal contacts view when the box is cleared.
            var contactsTabBtn = document.querySelector('[data-side-tab="contacts"]');
            if (contactsTabBtn) contactsTabBtn.click();
            return;
          }
          searchDebounce = setTimeout(function () {
            fetch('/search-users?q=' + encodeURIComponent(q), { credentials: 'same-origin' })
              ...
              .catch(function () {
                if (contactsList) contactsList.innerHTML = '<div class="current-chat-status p-3">Search failed. Try again.</div>';
              });
          }, 300);
        });
      }
