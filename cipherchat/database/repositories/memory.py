...
class MemoryRepository(StoreBackend):
    ...
    def edit_message(self, message_id: str, new_body: str) -> bool:
        ...
        containers += [r.get("messages", []) for r in self.private_rooms.values() if isinstance(r, dict)]
        for messages in containers:
            for msg in messages:
                if msg.get("message_id") == message_id:
                    msg["message"] = new_body
                    msg["edited_at"] = __import__("datetime").datetime.utcnow().isoformat()
                    return True
        return False
