...
def verify_email(token: str):
    """Web fallback for email verification.

    Completes verification on the server, then offers a deep link into the
    Android app (cipherchat://verify/<token>).
    """
    import json
    ...
    flash(msg, "success" if ok else "error")
    app_link = f"cipherchat://verify/{token}"
    login_url = url_for("auth.login")
    title = "Email verified" if ok else "Verification"
    safe_msg = (msg or "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    app_js = json.dumps(app_link)
    html = f"""<!DOCTYPE html>
<html><head>
<meta charset="utf-8">
...
<script>
  setTimeout(function() {{ window.location.href = {app_js}; }}, 80);
</script>
</head>
<body style="font-family:system-ui,sans-serif;background:#0b0f1a;color:#eee;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0">
  <div style="text-align:center;max-width:420px;padding:24px">
    <h2>{title}</h2>
    <p>{safe_msg}</p>
    <p style="opacity:.85">Opening CipherChat Pro…</p>
    <p>
      <a href="{app_link}" style="color:#8b9cff">Open the app</a>
      &nbsp;·&nbsp;
      <a href="{login_url}" style="color:#8b9cff">Continue on web</a>
    </p>
  </div>
</body></html>"""
