...
def _fallback_redirect(fallback_endpoint: str) -> str:
    try:
...
def safe_next_redirect(next_page: Optional[str], fallback_endpoint: str = "chat.chat") -> str:
    ...
    fallback = _fallback_redirect(fallback_endpoint)
    if not next_page:
        return fallback
...
def is_temp_email(email: Optional[str]) -> bool:
    """Reject common disposable email providers."""
    from cipherchat.auth.validators import is_temp_email as _is_temp_email

    return bool(_is_temp_email(email))


def _find_by_email(email: str) -> Optional[str]:
