...
from cipherchat.auth.validators import is_valid_username, normalize_email
from cipherchat.config import get_config
from cipherchat.database.repositories.base import DuplicateUserError
...
_VERIFY_RESEND_LIMIT_PREFIX = "verify_resend:"
...
def _as_utc(value: datetime) -> datetime:
    """Normalize a stored expiry timestamp into an aware UTC datetime.

    Root-cause fix for the "Invalid or Expired" verification bug: expiry
    timestamps are always generated in UTC (see _utc_now()), but a naive
    datetime written to a timezone-aware DB column can come back from
    different backends either as UTC-aware (PostgreSQL) or as a naive value
    that still represents UTC (SQLite, or older rows). Comparing either of
    those against a *local* datetime.now() silently shifts the effective
    expiry by the server's UTC offset, which is exactly what caused valid,
    unexpired links to be rejected (or let expired ones through) depending
    on the deployment's timezone. Treating a naive value as UTC — instead of
    stripping tzinfo off an aware one — keeps every comparison in one
    unambiguous timezone.
    """
...
def resolve_user_email(username: str) -> Optional[str]:
    ...
    for uname, em in list(store.username_index.items()):
        if uname.lower() == key:
            return em
    # Final fallback: ask the backend directly. The in-memory index above is
    # a per-process cache that can be stale or incomplete (e.g. multiple
    # worker processes behind PostgreSQL), so treat the backend as the
    # authoritative source rather than reporting a false "not taken".
    try:
        return store.backend.resolve_email(username)
    except Exception:
        return None
...
def resolve_login_identifier(identifier: str) -> Optional[str]:
    """
    Resolve a single login identifier (username OR email) to account email.
    """
...
def register_user(
    ...
    # Fast-path duplicate checks. These are a UX optimization only — the
    # authoritative, race-safe check happens in store.backend.create_user()
    # below, which is backed by DB-level unique constraints.
    ...
    # Create pending account. create_user() is an atomic insert enforced by
    # DB-level uniqueness on email and (case-insensitive) username, so even
    # if two requests race past the checks above at the same instant, only
    # one of them can actually create the account — the loser gets a clean
    # DuplicateUserError instead of a silently overwritten/duplicated user.
    try:
        store.backend.create_user(email, user_record)
    except DuplicateUserError as exc:
        if exc.field == "username":
            logger.info("Registration rejected: username already taken (race)")
            return False, "Username is already taken."
        logger.info("Registration rejected: email already registered (race)")
        return False, "An account with this email already exists."
    except Exception:
    ...
    store.verification_tokens[token] = {
        "email": email,
        "expires": _utc_now() + timedelta(hours=24),
    }
...
def confirm_email_token(token: str) -> Tuple[bool, str, Optional[str]]:
    ...
    Token is single-use: it is removed from storage as soon as it is looked
    up here, whether it turns out to be expired, orphaned, or valid — a
    token can never be replayed after this function has evaluated it once.
    """
    data = store.verification_tokens.get(token)
    if not data:
        return False, "Invalid or expired verification link.", None
    # Consume immediately so a token can never be evaluated twice, even if
    # something below fails.
    store.verification_tokens.pop(token, None)

    expires = _as_utc(data["expires"])
    if _utc_now() > expires:
...
def resend_verification_email(email: str, base_url: str) -> Tuple[bool, str]:
    """Issue a fresh verification token and invalidate any previous ones.

    Always returns a generic-sounding success message to avoid confirming
    whether an account exists, mirroring request_password_reset()'s pattern.
    """
    email = normalize_email(email)
    generic = "If that email is pending verification, a new verification link has been sent."

    resend_key = _VERIFY_RESEND_LIMIT_PREFIX + (email or "unknown")
    if is_rate_limited(resend_key, max_attempts=5, window_hours=1):
        return True, generic
    ...
    if not user or user.get("verified"):
        # Unknown account or already verified — nothing to resend.
        return True, generic

    # Invalidate every outstanding token for this email first, so an old
    # link can never be used after a new one is requested, and so a user
    # never ends up with more than one active verification token.
    store.backend.delete_verification_tokens_for_email(email)

    token = generate_token(24)
    expires_at = _utc_now() + timedelta(hours=24)
    verify_url = f"{base_url.rstrip('/')}/verify/{token}"
    try:
        html_body = create_verification_email(verify_url, user.get("username", ""), email)
    except Exception:
        logger.exception("Verification email template failed during resend")
        return True, generic

    result = send_email_result(email, "Verify your CipherChat account", html_body)
    if not result.success:
        logger.warning("Resend verification email failed (code=%s)", result.error_code)
        return True, generic

    store.verification_tokens[token] = {"email": email, "expires": expires_at}
    add_rate_limit(resend_key)
