...
def _ensure_admin(cfg, store) -> None:
    ...
    if admin_email in store.users:
        data = store.users[admin_email]
        data.update(
            {
                "username": "admin",
                "status": "approved",
    ...
    # Promote username-admin accounts (custom email but admin handle)
    for em, data in list(store.users.items() if hasattr(store.users, "items") else []):
        if (data.get("username") or "").lower() == "admin":
            data["role"] = "admin"
