...
def serialize_group(group: dict, viewer_email: str | None = None) -> dict:
    ...
    data = {
        ...
        "settings": {
            # Only settings the backend actually enforces are exposed here —
            # see chat/groups/services.py for where each is checked.
            "members_can_add": bool(group.get("members_can_add", False)),
        },
    }
    if viewer_email:
